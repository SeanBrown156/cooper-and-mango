...
├── music/
│   └── <theme-name>/
│       ├── source/        (Ableton project files, stems — gitignored via raw/reference pattern if large; confirm LFS decision first)
...
- Raw Ableton projects and unprocessed pet-sound recordings live under `raw/reference/` per the same provenance rule as visual reference material (see `ART_BIBLE.md` and the open Git LFS question in the project plan).
